...
def build_libanon_order_turn_test_tool_config(
    ...
    return {
        ...
        "description": (
            ...
            "information question such as ingredients or opening hours. The "
...
def build_yz_test_menu_resolver_v2_tool_config(
    ...
    return {
        ...
        "description": (
            ...
            "A stated menu number is also a product attempt: the backend "
            "resolves a unique numbered dish, asks for protein when that "
            "number has verified protein variants, and asks for a required "
            "size when the menu number has sizes. "
            "The backend reads the latest raw customer utterance from "
            "the conversation history and returns only a verified active "
            "menu item: exact canonical name, explicitly approved alias, "
            "or one uniquely high-confidence deterministic spelling "
            "match. Do not call it for a modifier-only, quantity-only, "
            "or yes/no answer. "
